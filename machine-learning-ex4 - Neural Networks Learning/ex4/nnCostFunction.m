...
J = 0;
Theta_I_grad = zeros(size(Theta1));
Theta_II_grad = zeros(size(Theta2));
...
%         Theta_I_grad and Theta_II_grad. You should return the partial derivatives of
%         the cost function with respect to Theta1 and Theta2 in Theta_I_grad and
%         Theta_II_grad, respectively. After implementing Part 2, you can check
...
%               the regularization separately and then add them to Theta_I_grad
%               and Theta_II_grad from Part 2.
%


% 1. hacia adelanta para tener h = a3.
...
% Vectorized ftw:
Theta_II_grad = (1/m) * d3 * a2';
Theta_I_grad = (1/m) * d2(2:end, :) * a1';

% Add gradient regularization.
Theta_II_grad = Theta_II_grad + ...
              (lambda / m) * ([zeros(size(Theta2, 1), 1), Theta2(:, 2:end)]);
Theta_I_grad = Theta_I_grad + ...
              (lambda / m) * ([zeros(size(Theta1, 1), 1), Theta1(:, 2:end)]);

% Unroll gradients.
grad = [Theta_I_grad(:) ; Theta_II_grad(:)];
